...
func (c *Docker) GetStats(id string) (Stats, error) {
	var stats Stats
	resp, err := c.client.Get("http://unix/v1.24/containers/" + id + "/stats?stream=0")
	if err != nil {
		return stats, err
	}
...
func (c *Docker) GetConcurrentStats(container APIContainers, ch chan<- ContainerStats) {
	var cs ContainerStats
	cs.info = container
	resp, err := c.client.Get("http://unix/v1.24/containers/" + container.ID + "/stats?stream=0")
	if err != nil {
		log.Println(err)
		return
	}
	defer resp.Body.Close()
	if err := json.NewDecoder(resp.Body).Decode(&cs.stats); err != nil {
		log.Println(err)
		return
	}
	ch <- cs
