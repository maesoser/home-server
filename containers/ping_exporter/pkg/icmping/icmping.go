...
type Pinger struct {
	...
	// ttls is all of the received ttls
	ttls []int
...
type Statistics struct {
	...
	// MaxTTL is the maximum TimeToLive time received via this pinger.
	MaxTTL int
	// MinTTL is the minimum TimeToLive time received via this pinger.
	MinTTL int
}
	...
	for {
		...
		default:
			...
			if p.ipv4 {
				...
				n, cm, peer, err = conn.IPv4PacketConn().ReadFrom(bytes)
				if cm != nil {
					ttl = cm.TTL
				}
			} else {
				var cm *ipv6.ControlMessage
				n, cm, peer, err = conn.IPv6PacketConn().ReadFrom(bytes)
				if cm != nil {
					ttl = cm.HopLimit
				}
			}
			...
			recv <- &packet{bytes: bytes, nbytes: n, ttl: ttl, src: peer}
		}
	}
}
